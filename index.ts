...
var received_message: any;
var received_from: any;
var reply_message: any;
var reply_id: string;
let client:any;
...
function ws_connect(){
    ...
    client.on('connect', function (connection: any) {
        ...
        connection.on('message', function (raw_message: any) {
                            ...
                            if (received_message != null) {
                                if (received_message.includes("サイコロ振って")) {
                                    ...
                                    if (received_message.match(/[０-９]/)) {
                                        ...
                                        let raw_dice_roll: any = received_message.match(/[0-9]{1,}d[0-9]{1,}/)[0];
                                        let dice_roll = raw_dice_roll.match(/[0-9]{1,}/g);
                                        let times: any = dice_roll[0];
                                        let range: any = dice_roll[1];
...
//Misskeyにnoteを投稿する
async function post_note(message: string, is_reply: boolean, reply_id: string) {
    var options: any;
    if (is_reply) {
        options = {
            ...
            json: {
                i: TOKEN,
                visibility: "public", //公開時はpublic,非公開時はhome
                "localOnly": false,
                text: message,
                replyId: reply_id
            }
...
async function get_fortune(date_url: string, horoscope: any) {
